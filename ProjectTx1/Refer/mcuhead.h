...
#define _16T8H(x) (((union u2i*)&x)->c[0])//16位转高8位
#define _16T8L(x) (((union u2i*)&x)->c[1])//??16??8?

#define _32T16H(x) (((union u2i*)&x)->li[0])//??32??16?
#define _32T16L(x) (((union u2i*)&x)->li[1])//??32??16?

#define _32T8HH(x) (((union u2i*)&x)->lc[0])//??32??8?
#define _32T8H(x) (((union u2i*)&x)->lc[1])//??32???8?
#define _32T8L(x) (((union u2i*)&x)->lc[2])//??32???8?
#define _32T8LL(x) (((union u2i*)&x)->lc[3])//??32?8?
...
#define _GPIO_PP(io,x) io##_DDR|=x;io##_CR1|=x;io##_CR2&=~(x);
#define GPIO_PP(io,x) _GPIO_PP(io,BIN(x))

#define _GPIO_OD(io,x) io##_DDR|=x;io##_CR1&=~(x);io##_CR2&=~(x);
#define GPIO_OD(io,x) _GPIO_OD(io,BIN(x))

#define _GPIO_AF(io,x) io##_DDR&=~(x);io##_CR1&=~(x);io##_CR2&=~(x);
#define GPIO_AF(io,x) _GPIO_AF(io,BIN(x))

#define _GPIO_PU(io,x) io##_DDR&=~(x);io##_CR1|=x;io##_CR2&=~(x);
#define GPIO_PU(io,x) _GPIO_PU(io,BIN(x))

#define _GPIO_INT(io,x) io##_DDR&=~(x);io##_CR1|=x;io##_CR2|=x;
#define GPIO_INT(io,x) _GPIO_INT(io,BIN(x))

#define _GPIO_AF_INT(io,x) io##_DDR&=~(x);io##_CR1&=~(x);io##_CR2|=x;
#define GPIO_AF_INT(io,x) _GPIO_AF_INT(io,BIN(x))
